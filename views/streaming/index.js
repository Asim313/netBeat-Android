...
export default class App extends Component<Props> {
                ...
                this.getToken()
                    .then(async (token) => {
                      ...
                      const a = await _retrieveData('user');
                        ...
                        mySession
                            .connect(token, JSON.stringify({ clientData: a?.id,username : a?.username ,group_id : a?.group?.id,type : 'vr'}))
                            .then(() => {
    ...
    render() {
        ...
        {/* <Animated.View style={{ alignSelf : 'flex-start', justifyContent : 'center',alignItems : 'center',  paddingHorizontal : wp(3), paddingVertical : wp(0.5), borderRadius : wp(10), top : this.state.topHeaderHeight}}>
         ...
         <TouchableOpacity onPress={()=>{this.props.navigation.pop()}} style={{paddingHorizontal : wp(2), paddingVertical : hp(5)}}>
